...
def execute(g,x):
    ...
    for n in g.nodes:
        ...
        elif n.op=="const_-1": z=torch.full_like(x[:,0],-1.0)
        elif n.op=="const_1": z=torch.full_like(x[:,0],1.0)
        elif n.op=="const_2": z=torch.full_like(x[:,0],2.0)
        elif n.op=="const_3": z=torch.full_like(x[:,0],3.0)
...
def candidate_graphs(task,depth=3,max_nodes=10):
    ...
    # Compact constant + input constructions: x +/- c and x * c.
    for cop in consts:
        for i in range(len(base)):
            ns=base+[Node(cop,()),Node("add",(i,len(base)))]
            if len(ns)<=max_nodes: out.append(Graph(tuple(ns),len(ns)-1))
            ns=base+[Node(cop,()),Node("sub",(i,len(base)))]
            if len(ns)<=max_nodes: out.append(Graph(tuple(ns),len(ns)-1))
            ns=base+[Node(cop,()),Node("mul",(i,len(base)))]
            if len(ns)<=max_nodes: out.append(Graph(tuple(ns),len(ns)-1))
    ...
    if depth>=2:
        first=[]
        for i in range(len(base)):
            for op in unary: first.append(Node(op,(i,)))
        for i in range(len(base)):
            for j in range(len(base)):
                for op in binary: first.append(Node(op,(i,j)))
        for f in first:
    ...
    # Explicit affine representation for the current compose source task:
    # (2*x0 + 1) - (3*x1 - 1)
    if task=="compose" and max_nodes>=10:
        nodes=[
            Node("input0"),Node("input1"),
            Node("const_1"),Node("const_2"),Node("const_3"),
            Node("mul",(0,3)),
            Node("add",(5,2)),
            Node("mul",(1,4)),
            Node("sub",(7,2)),
        ...
        out.append(Graph(tuple(nodes),9))

    if depth>=3 and arity==3:
...
def run_seed(seed,args,device,idx):
    ...
    for i,t in enumerate(source):
        ...
        if got is None:
            source_rows.append({"task":t,"status":"SOURCE_DISCOVERY_FAILED","candidate_count":len(candidate_graphs(t,args.max_graph_depth,args.max_graph_nodes)),"reason":"no_exact_verified_candidate_in_affine_aware_language"})
...
def main():
    ...
    p.add_argument("--max-program-length",type=int,default=2); p.add_argument("--max-graph-depth",type=int,default=3); p.add_argument("--max-graph-nodes",type=int,default=10)
    ...
    p.add_argument("--device",default="cuda"); p.add_argument("--out",default="repair01.result.json"); a=p.parse_args()
    ...
    Path(a.out).write_text(json.dumps(result,indent=2)); print("DART-4.3 repair01: affine-aware source primitive discovery"); print(json.dumps(result,indent=2)); print(f"Saved: {Path(a.out).resolve()}")
