#!/usr/bin/env python3
"""DART-0.8: Routing-Preserving Shared Computation.
...
DART-0.8 therefore changes one thing only:
...
The attention modules and both LayerNorms are copied from the teacher and
frozen during candidate search. This isolates whether preserving information
routing is sufficient to prevent the capability collapse seen in DART-0.6/0.7.
...
def candidate_search(base, train_loader, verifier_loader, reference_routing, device, args, seed):
    out=[]
...
def run_one(args,seed,task):
    ...
    for r in range(args.surgery_rounds):
        cand=candidate_search(dart,tr,va,ref_route,device,args,seed+1000*r); win=cand[0]
        ...
        rounds.append({"round":r,"winner":win,"all_candidates":cand,"pre_adapt":asdict(pre),"post_adapt":asdict(post),"routing_agreement_after":route,"adaptation_seconds":adapt,"fit_seconds":fit_secs,"latency_ms":cuda_latency(dart,te,device,args.latency_warmup,args.latency_iters)})
...
def run_transfer(args,seed,source_task,target_task):
    ...
    cand=candidate_search(source,src,src,ref,device,args,seed+7000)[0]
...
def main():
    ...
    p.add_argument("--batch-size",type=int,default=256); p.add_argument("--latency-warmup",type=int,default=30); p.add_argument("--latency-iters",type=int,default=30); p.add_argument("--device",default="cuda" if torch.cuda.is_available() else "cpu"); p.add_argument("--out",default="dart08_results.json")
    a=p.parse_args(); records=[]; print("DART-0.8: routing-preserving replacement",flush=True)
    for task in a.tasks:
    ...
    Path(a.out).write_text(json.dumps(out,indent=2),encoding="utf-8"); print("\n================ DART-0.8 SUMMARY ================"); print(out["summary"]); print(f"Saved: {Path(a.out).resolve()}")
