...
def verify(task,g,seed,args,device):
    ...
    scores["proof"]=min([scores["symbolic"],*regimes.values(),*rnd.values()])
    return scores
...
def run_audit(task,seed,args,device,lib):
    ...
    verified=[(g,v) for g,v in rows if v["proof"]==1.0]
    expected_v=next((v for g,v in rows if g.key()==exp.key()),None)

    inserted=False
    reused=False
    pid=None
    if expected_v is not None and expected_v["proof"]==1.0:
    ...
    elif expected_v["proof"] < 1.0: anomaly.append("expected_graph_failed_exact_verification")
    if best_v["proof"] < 1.0: anomaly.append("no_exact_candidate_in_current_language")
    if expected_v is not None and expected_v["proof"]==1.0 and not inserted and not reused:
...
def main():
    ...
    ap.add_argument("--out",default="repair.result.json")
    args=ap.parse_args()
    ...
    expected_verified=sum(
        ...
        if row["expected_graph_verification"] and row["expected_graph_verification"]["proof"]==1.0
    )
    ...
    result={
        ...
        "purpose":"primitive discovery pipeline integrity",
        "gates":{
    ...
    print("DART-4.1 repair01: primitive discovery pipeline integrity")
