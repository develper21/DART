#!/usr/bin/env python3
"""DART-2.7: causal primitive localization + distributed mediation discovery.
...
class Progress:
    ...
    def show(self, frac, phase, detail=''):
        ...
        msg=f"\r[DART-2.7][seed {self.seed_idx}/{self.seeds}] [{bar}] {self.current*100:6.2f}% | {phase}"
...
def run_seed(args,seed,seed_idx=1,total_seeds=1):
        ...
        for nodes in node_families:
            for mode in modes:
                for mask in placements:
                    cand_idx+=1
                    start=0.04+0.75*(cand_idx-1)/max(1,total_candidates); span=0.75/max(1,total_candidates)
                    tracker.show(start,'candidate-search',f'{cand_idx}/{total_candidates} | {motif}/{mode} | mask={mask}')
                    ...
                    for r in rules:
                        ...
                        src_active=[s for s in range(3) if not isinstance(r.adapters[s],NoOpAdapter)]
                        dst_active=[s for s in range(3) if not isinstance(rr.adapters[s],NoOpAdapter)]
                        for ss,dd in zip(src_active,dst_active): ddst=rr.adapters[dd]; ddst.load_state_dict(r.adapters[ss].state_dict())
                    ...
                    rgap=sum(acc)/len(acc)-sum(racc)/len(racc); placement_gain=rgap
                    perm=[]
                    for i,t in enumerate(meta):
                        row=[]
                    ...
                    med_need=sum(m['necessity'] for m in mediation)/len(mediation)
                    med_suff=sum(m['sufficiency'] for m in mediation)/len(mediation)
                    traj=sum(m['trajectory_fidelity'] for m in mediation)/len(mediation)
                    min_alpha=sum(m['minimal_alpha'] for m in mediation)/len(mediation)
                    minimality=sum(m['minimality'] for m in mediation)/len(mediation)
                    cme=sum(m['cme'] for m in mediation)/len(mediation)
                    conc=sum(m['causal_concentration'] for m in mediation)/len(mediation)
                    red=sum(m['redundancy_index'] for m in mediation)/len(mediation)
                    syn=sum(m['synergy_index'] for m in mediation)/len(mediation)
                    min_set_size=sum(m['minimal_set_size'] for m in mediation)/len(mediation)
                    cross_overlap=cross_task_causal_overlap(mediation)
                    random_set_gap=0.0
                    for i,t in enumerate(meta):
                        ...
                        best=max(mediation[i]['set_scores'].values()) if mediation[i]['set_scores'] else 0.0
                        random_set_gap += best-rand
                    random_set_gap/=len(meta)
                    active_k=sum(mask); budget_penalty=args.budget_lambda*active_k
                    eligible=(sum(acc)/len(acc)>=args.min_avg_source_acc and min(acc)>=args.min_worst_source_acc and stab<=args.max_adapter_stability and pg>=args.min_operator_specificity and rgap>=args.min_random_placement_gap and parity<=args.max_shared_vs_separate_gap and op_eff>=args.min_adapter_effect and med_need>=args.min_mediation_necessity and med_suff>=args.min_mediation_sufficiency and traj>=args.min_intervention_fidelity and cme>=args.min_cme and cross_overlap>=args.min_cross_task_overlap and random_set_gap>=args.min_random_set_gap and min_set_size<=args.max_minimal_set_size)
                    score=sum(acc)/len(acc)+args.rule_weight*med_need+args.operator_weight*pg+args.random_weight*rgap+args.placement_weight*placement_gain+args.mediation_weight*med_suff+args.cme_weight*cme+args.localization_weight*conc+args.cross_task_weight*cross_overlap+args.random_set_weight*random_set_gap-args.parity_weight*max(0,parity)-args.budget_weight*budget_penalty-args.complexity_lambda*(count_params(sk)+sum(count_params(a) for r in rules for a in r.adapters if not isinstance(a,NoOpAdapter)))
                    tracker.show(start+span*0.95,'candidate-controls',f'{cand_idx}/{total_candidates} | MCS={min_set_size:.1f} conc={conc:.3f} syn={syn:.3f}')
                    candidates.append({'motif':motif,'nodes':nodes,'operator_mode':mode,'placement':mask,'random_placement':rmask,'source_avg':sum(acc)/len(acc),'source_worst':min(acc),'adapter_stability':stab,'adapter_effect':op_eff,'rule_causal_fidelity':rcf,'operator_specificity':pg,'random_placement_gap':rgap,'separate_graph_avg':sepavg,'shared_vs_separate_gap':parity,'placement_gain':placement_gain,'slot_causal_necessity':sum(m.get('necessity',0) for m in mediation)/len(mediation),'mediation_necessity':med_need,'mediation_sufficiency':med_suff,'trajectory_fidelity':traj,'minimal_alpha':min_alpha,'minimality':minimality,'cme':cme,'causal_concentration':conc,'redundancy_index':red,'synergy_index':syn,'minimal_causal_set_size':min_set_size,'cross_task_causal_overlap':cross_overlap,'random_causal_set_gap':random_set_gap,'mediation_slot_scores':[m['slot_scores'] for m in mediation],'mediation_set_scores':[m['set_scores'] for m in mediation],'minimal_sets':[m['minimal_set'] for m in mediation],'active_adapters':active_k,'budget_penalty':budget_penalty,'task_acc':acc,'operator_permutation_matrix':perm,'eligible':eligible,'score':score,'rules':rules,'skeleton':sk})
    ...
    for label,t in [('related',args.holdout_tasks[0]),('contrast',contrast)]:
        ...
        src_states=[fitted.adapters[s].state_dict() for s in range(3) if not isinstance(fitted.adapters[s],NoOpAdapter)]; k=0
        for s in range(3):
            if isinstance(wrong.adapters[s],NoOpAdapter): continue
            wrong.adapters[s].load_state_dict(src_states[k]); k+=1
        ...
        results[label+'_holdout']={'task':t,'teacher':evaluate(tea,te,device),'dart_zero':zero,'dart_adapted':adapted,'placement_control':pacc,'mlp_control':am,'gain_zero':(zero-evaluate(tea,te,device))*100,'gain_adapt':(adapted-evaluate(tea,te,device))*100,'vs_mlp_adapt':(adapted-am)*100}
    tracker.show(0.99,'holdout-transfer',f'target={args.holdout_tasks[0]} contrast={contrast}')
...
def main():
    p=argparse.ArgumentParser(description='DART-2.7 causal primitive localization + distributed mediation discovery')
    ...
    summary={'related_holdout':{args.holdout_tasks[0]:{k:av(k,'related_holdout') for k in ['teacher','dart_zero','dart_adapted','placement_control','mlp_control','gain_zero','gain_adapt','vs_mlp_adapt']}},'contrast_holdout':{args.contrast_tasks[0]:{k:av(k,'contrast_holdout') for k in ['teacher','dart_zero','dart_adapted','placement_control','mlp_control','gain_zero','gain_adapt','vs_mlp_adapt']}},'source':{
        'avg_accuracy':sum(r['winner']['source_avg'] for r in rec)/len(rec),
        'avg_adapter_stability':sum(r['winner']['adapter_stability'] for r in rec)/len(rec),
        'avg_adapter_effect':sum(r['winner']['adapter_effect'] for r in rec)/len(rec),
        'avg_rule_causal_fidelity':sum(r['winner']['rule_causal_fidelity'] for r in rec)/len(rec),
        'avg_operator_specificity':sum(r['winner']['operator_specificity'] for r in rec)/len(rec),
        'avg_random_placement_gap':sum(r['winner']['random_placement_gap'] for r in rec)/len(rec),
        'avg_placement_gain':sum(r['winner']['placement_gain'] for r in rec)/len(rec),
        'avg_separate_graph_accuracy':sum(r['winner']['separate_graph_avg'] for r in rec)/len(rec),
        'avg_shared_vs_separate_gap':sum(r['winner']['shared_vs_separate_gap'] for r in rec)/len(rec),
        'avg_mediation_necessity':sum(r['winner'].get('mediation_necessity',0.0) for r in rec)/len(rec),
        'avg_mediation_sufficiency':sum(r['winner'].get('mediation_sufficiency',0.0) for r in rec)/len(rec),
        'avg_trajectory_fidelity':sum(r['winner'].get('trajectory_fidelity',0.0) for r in rec)/len(rec),
        'avg_minimal_alpha':sum(r['winner'].get('minimal_alpha',1.0) for r in rec)/len(rec),
        'avg_minimality':sum(r['winner'].get('minimality',0.0) for r in rec)/len(rec),
        ...
        'avg_redundancy_index':sum(r['winner'].get('redundancy_index',0.0) for r in rec)/len(rec),
        'avg_synergy_index':sum(r['winner'].get('synergy_index',0.0) for r in rec)/len(rec),
        ...
        'avg_random_causal_set_gap':sum(r['winner'].get('random_causal_set_gap',0.0) for r in rec)/len(rec),
        'avg_active_adapters':sum(r['winner'].get('active_adapters',sum(r['winner'].get('placement',[]))) for r in rec)/len(rec),
        'avg_budget_penalty':sum(r['winner'].get('budget_penalty',0.0) for r in rec)/len(rec)}}
    payload={'config':vars(args),'records':rec,'summary':summary}; Path(args.out).write_text(json.dumps(payload,indent=2)); print('DART-2.7: causal primitive localization + distributed mediation discovery'); print(json.dumps(summary,indent=2)); print('Saved:',Path(args.out).resolve())
