...
6) repair.result.json stores the complete repair01 result and protocol metadata.
"""
...
class Progress:
    ...
    def update(self,done,label,detail=""):
        ...
        sys.stdout.write(f"\r[DART-3.0][repair01][seed {self.seed_idx}/{self.nseeds}] [{bar}] {100*frac:6.2f}% | {label}")
...
def main():
    ...
    ap.add_argument('--out',default='repair.result.json'); args=ap.parse_args()
    ...
    for si,seed in enumerate(args.seeds,1):
        ...
        for m,t in zip(src_program_models,sources):
            x_probe,y_probe=make_data(t,args.causal_probe_size,seed+8000+t.__hash__()%1000,device)
            effects.extend(necessity_same_state(m,x_probe,y_probe))
        necessity=sum(effects)/len(effects) if effects else 0.0
        # Target adaptation/validation/test split. Test is untouched until final selection.
        ...
        zero_model=task_fit_frozen(base,Program(('identity',)),target_tr[0],target_tr[1],max(1,args.target_program_fit_steps//4),args.lr)
        zero_acc=acc(zero_model,*target_te)
        best_target=None
        for i,pr in enumerate(programs):
            m=task_fit_frozen(base,pr,target_tr[0],target_tr[1],args.target_program_fit_steps,args.lr)
            va_acc=acc(m,*target_va); test_acc=acc(m,*target_te)
            score=va_acc-args.complexity_lambda*len(pr.ops)
            if best_target is None or score>best_target[0]: best_target=(score,test_acc,va_acc,pr)
            pg.update(start_prog+len(programs)+i,'target-program-validation',f'program={i+1}/{len(programs)} val={va_acc:.3f}')
        # Permutation/random controls are fit on target adaptation and evaluated on untouched test.
        wrong=Program(('negate',)) if best_pr.ops[0] != 'negate' else Program(('scale',))
        pm=task_fit_frozen(base,wrong,target_tr[0],target_tr[1],args.transfer_control_steps,args.lr); perm=acc(pm,*target_te)
        rp=programs[(seed*13)%len(programs)]; rm=task_fit_frozen(base,rp,target_tr[0],target_tr[1],args.transfer_control_steps,args.lr); rnd=acc(rm,*target_te)
        ...
        cm=task_fit_frozen(base,best_pr,c_tr[0],c_tr[1],args.transfer_control_steps,args.lr); cacc=acc(cm,*c_te)
        pg.close()
        records.append({
            'seed':seed,
            'winner':{'motif':motif,'nodes':nodes,'program':list(best_pr.ops),'source_avg_balanced':float(bal),'source_mean_accuracy':float(src_mean),'source_worst_accuracy':float(src_worst),'source_verifier_accuracies':src_verifier_accs,'program_source_mean':float(prog_mean),'program_source_worst':float(prog_worst),'program_source_task_accuracies':src_program_vals,'program_necessity':float(necessity)},
            'related_holdout':{target:{'teacher':teacher_scores[target],'dart_zero':float(zero_acc),'dart_program':float(best_target[1]),'program_validation':float(best_target[2]),'program_permutation_control':float(perm),'random_program_control':float(rnd)}},
            'contrast_holdout':{contrast:{'teacher':teacher_scores[contrast],'dart_program':float(cacc)}}
        })
    summary={
        'repair_id':'repair01','parent_version':'DART-3.0',
        'repairs':{
    ...
    out=Path(args.out); out.write_text(json.dumps(summary,indent=2)); print('DART-3.0 repair01: evaluation integrity repair'); print(json.dumps(summary,indent=2)); print('Saved:',out.resolve())
