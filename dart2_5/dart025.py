...
class Progress:
    ...
    def show(self, frac, phase, detail=""):
        ...
        msg=f"\r[DART-2.5][seed {self.seed_idx}/{self.seeds}] [{bar}] {self.current*100:6.2f}% | {phase}"
        ...
        for nodes in node_families:
            for mode in modes:
                for mask in placements:
                    ...
                    reconstruction_fidelity, reconstruction_slot_scores = slot_reconstruction_fidelity(rules, teachers, meta, args, device)
                    active_k=sum(mask)
                    budget_penalty=args.budget_lambda*active_k
                    eligible=(sum(acc)/len(acc)>=args.min_avg_source_acc and min(acc)>=args.min_worst_source_acc and stab<=args.max_adapter_stability and pg>=args.min_operator_specificity and rgap>=args.min_random_placement_gap and parity<=args.max_shared_vs_separate_gap and op_eff>=args.min_adapter_effect and slot_causal_avg>=args.min_slot_causal_necessity and reconstruction_fidelity>=args.min_slot_reconstruction_fidelity)
                    score=sum(acc)/len(acc)+args.rule_weight*slot_causal_avg+args.operator_weight*pg+args.random_weight*rgap+args.placement_weight*placement_gain+args.reconstruction_weight*reconstruction_fidelity-args.parity_weight*max(0,parity)-budget_penalty-args.complexity_lambda*(count_params(sk)+sum(count_params(a) for r in rules for a in r.adapters if not isinstance(a,NoOpAdapter)))
                    tracker.show(start_frac+span*0.95, 'candidate-controls', f'{cand_idx}/{total_candidates} | slot={slot_causal_avg:.3f} recon={reconstruction_fidelity:.3f}')
                    candidates.append({'motif':motif,'nodes':nodes,'operator_mode':mode,'placement':mask,'random_placement':rmask,'source_avg':sum(acc)/len(acc),'source_worst':min(acc),'adapter_stability':stab,'adapter_effect':op_eff,'rule_causal_fidelity':rcf,'operator_specificity':pg,'random_placement_gap':rgap,'separate_graph_avg':sepavg,'shared_vs_separate_gap':parity,'placement_gain':placement_gain,'slot_causal_necessity':slot_causal_avg,'slot_causal_scores':slot_causal_scores,'slot_reconstruction_fidelity':reconstruction_fidelity,'slot_reconstruction_scores':reconstruction_slot_scores,'active_adapters':active_k,'budget_penalty':budget_penalty,'task_acc':acc,'operator_permutation_matrix':perm,'eligible':eligible,'score':score,'rules':rules,'skeleton':sk})
...
def main():
    ...
    summary={'related_holdout':{args.holdout_tasks[0]:{k:av(k,'related_holdout') for k in ['teacher','dart_zero','dart_adapted','placement_control','mlp_control','gain_zero','gain_adapt','vs_mlp_adapt']}},'contrast_holdout':{args.contrast_tasks[0]:{k:av(k,'contrast_holdout') for k in ['teacher','dart_zero','dart_adapted','placement_control','mlp_control','gain_zero','gain_adapt','vs_mlp_adapt']}},'source':{'avg_accuracy':sum(r['winner']['source_avg'] for r in rec)/len(rec),'avg_adapter_stability':sum(r['winner']['adapter_stability'] for r in rec)/len(rec),'avg_adapter_effect':sum(r['winner']['adapter_effect'] for r in rec)/len(rec),'avg_rule_causal_fidelity':sum(r['winner']['rule_causal_fidelity'] for r in rec)/len(rec),'avg_operator_specificity':sum(r['winner']['operator_specificity'] for r in rec)/len(rec),'avg_random_placement_gap':sum(r['winner']['random_placement_gap'] for r in rec)/len(rec),'avg_placement_gain':sum(r['winner']['placement_gain'] for r in rec)/len(rec),'avg_separate_graph_accuracy':sum(r['winner']['separate_graph_avg'] for r in rec)/len(rec),'avg_shared_vs_separate_gap':sum(r['winner']['shared_vs_separate_gap'] for r in rec)/len(rec),'avg_slot_causal_necessity':sum(r['winner'].get('slot_causal_necessity',0.0) for r in rec)/len(rec),'avg_slot_reconstruction_fidelity':sum(r['winner'].get('slot_reconstruction_fidelity',0.0) for r in rec)/len(rec),'avg_active_adapters':sum(r['winner'].get('active_adapters',sum(r['winner'].get('placement',[]))) for r in rec)/len(rec),'avg_budget_penalty':sum(r['winner'].get('budget_penalty',0.0) for r in rec)/len(rec)} }
    payload={'config':vars(args),'records':rec,'summary':summary}; Path(args.out).write_text(json.dumps(payload,indent=2)); print('DART-2.5: causal slot reconstruction + adaptive task budget'); print(json.dumps(summary,indent=2)); print('Saved:',Path(args.out).resolve())
