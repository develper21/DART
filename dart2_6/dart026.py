...
unbounded graph surgery are used. DART-2.5 additionally reconstructs each active
slot operation against teacher-grounded intervention effects and searches K in
{1, 2} active slots with an explicit budget penalty.
"""
...
class Progress:
    ...
    def show(self, frac, phase, detail=""):
        ...
        msg=f"\r[DART-2.6][seed {self.seed_idx}/{self.seeds}] [{bar}] {self.current*100:6.2f}% | {phase}"
            ...
            for mode in modes:
                for mask in placements:
                    ...
                    for i,t in enumerate(meta):
                        ...
                        mediation.append(causal_mediation_metrics(rules[i],teachers[t][0],ds,args,device,args.trajectory_start))
                    med_need=sum(m['necessity'] for m in mediation)/len(mediation)
                    med_suff=sum(m['sufficiency'] for m in mediation)/len(mediation)
                    traj_fid=sum(m['trajectory_fidelity'] for m in mediation)/len(mediation)
...
def main():
    p=argparse.ArgumentParser(description='DART-2.5 causal slot reconstruction + adaptive task budget')
    ...
    floats={'adapter-delta':0.1,'adapter-l2':0.0005,'adapter-lr':0.01,'min-avg-source-acc':.30,'min-worst-source-acc':.22,'max-adapter-stability':.75,'min-operator-specificity':.01,'min-adapter-effect':.02,'min-rule-causal-fidelity':0.0,'min-random-placement-gap':.02,'max-shared-vs-separate-gap':.03,'rule-weight':.30,'operator-weight':.20,'random-weight':.20,'placement-weight':.15,'parity-weight':.20,'min-slot-causal-necessity':.10,'min-slot-reconstruction-fidelity':0.20,'reconstruction-weight':0.30,'budget-lambda':0.03,'complexity-lambda':1e-5,'min-mediation-necessity':.10,'min-mediation-sufficiency':.20,'min-intervention-fidelity':.20,'min-cme':.01,'mediation-weight':.25,'cme-weight':.35,'budget-weight':1.0,'lr':.0003,'core-fit-lr':.001}
    for k,v in floats.items(): p.add_argument('--'+k,type=float,default=v)
    p.add_argument('--trajectory-start',type=int,default=0); p.add_argument('--trajectory-end',type=int,default=1); p.add_argument('--intervention-grid',nargs='+',type=float,default=[0.0,0.25,0.5,0.75,1.0]); p.add_argument('--device',default='cuda'); p.add_argument('--out',default='dart026_results.json')
    ...
    summary={'related_holdout':{args.holdout_tasks[0]:{k:av(k,'related_holdout') for k in ['teacher','dart_zero','dart_adapted','placement_control','mlp_control','gain_zero','gain_adapt','vs_mlp_adapt']}},'contrast_holdout':{args.contrast_tasks[0]:{k:av(k,'contrast_holdout') for k in ['teacher','dart_zero','dart_adapted','placement_control','mlp_control','gain_zero','gain_adapt','vs_mlp_adapt']}},'source':{'avg_accuracy':sum(r['winner']['source_avg'] for r in rec)/len(rec),'avg_adapter_stability':sum(r['winner']['adapter_stability'] for r in rec)/len(rec),'avg_adapter_effect':sum(r['winner']['adapter_effect'] for r in rec)/len(rec),'avg_rule_causal_fidelity':sum(r['winner']['rule_causal_fidelity'] for r in rec)/len(rec),'avg_operator_specificity':sum(r['winner']['operator_specificity'] for r in rec)/len(rec),'avg_random_placement_gap':sum(r['winner']['random_placement_gap'] for r in rec)/len(rec),'avg_placement_gain':sum(r['winner']['placement_gain'] for r in rec)/len(rec),'avg_separate_graph_accuracy':sum(r['winner']['separate_graph_avg'] for r in rec)/len(rec),'avg_shared_vs_separate_gap':sum(r['winner']['shared_vs_separate_gap'] for r in rec)/len(rec),'avg_mediation_necessity':sum(r['winner'].get('mediation_necessity',0.0) for r in rec)/len(rec),'avg_mediation_sufficiency':sum(r['winner'].get('mediation_sufficiency',0.0) for r in rec)/len(rec),'avg_trajectory_fidelity':sum(r['winner'].get('trajectory_fidelity',0.0) for r in rec)/len(rec),'avg_minimal_alpha':sum(r['winner'].get('minimal_alpha',1.0) for r in rec)/len(rec),'avg_minimality':sum(r['winner'].get('minimality',0.0) for r in rec)/len(rec),'avg_cme':sum(r['winner'].get('cme',0.0) for r in rec)/len(rec),'avg_slot_causal_necessity':sum(r['winner'].get('slot_causal_necessity',0.0) for r in rec)/len(rec),'avg_active_adapters':sum(r['winner'].get('active_adapters',sum(r['winner'].get('placement',[]))) for r in rec)/len(rec),'avg_budget_penalty':sum(r['winner'].get('budget_penalty',0.0) for r in rec)/len(rec)} }
    payload={'config':vars(args),'records':rec,'summary':summary}; Path(args.out).write_text(json.dumps(payload,indent=2)); print('DART-2.6: minimal causal mediation discovery'); print(json.dumps(summary,indent=2)); print('Saved:',Path(args.out).resolve())
