#!/usr/bin/env python3
"""DART-2.9: causal interface reparameterization around a frozen primitive + strict transfer.
...
class Progress:
    ...
    def show(self, frac, phase, detail=''):
        ...
        msg=f"\r[DART-2.9][seed {self.seed_idx}/{self.seeds}] [{bar}] {self.current*100:6.2f}% | {phase}"
...
class InterfaceAdapter(nn.Module):
    """Tiny bounded task-specific interface transform. Exactly two trainable scalars."""
    MODES=('affine','scale','shift','negate','difference','product')
    def __init__(self, mode='affine'):
        super().__init__(); self.mode=mode
        self.raw=nn.Parameter(torch.zeros(2,dtype=torch.float32))
    def forward(self,x,ref=None):
        if self.mode=='affine':
            a=1+0.5*torch.tanh(self.raw[0]); b=0.5*torch.tanh(self.raw[1]); return a*x+b
        if self.mode=='scale': return (1+0.5*torch.tanh(self.raw[0]))*x
        if self.mode=='shift': return x+0.5*torch.tanh(self.raw[0])
        if self.mode=='negate': return -(1+0.5*torch.tanh(self.raw[0]))*x
        if self.mode=='difference':
            r=ref if ref is not None else torch.zeros_like(x); a=0.5*torch.tanh(self.raw[0]); b=0.5*torch.tanh(self.raw[1]); return x-a*r+b
        if self.mode=='product':
            r=ref if ref is not None else x; a=0.5*torch.tanh(self.raw[0]); b=0.5*torch.tanh(self.raw[1]); return x*(1+a*torch.tanh(r))+b
        raise ValueError(self.mode)

class FrozenPrimitiveRule(nn.Module):
    """Shared skeleton + frozen primitive + DART-2.8 scalar reparameterization."""
    def __init__(self, skeleton, mode, base_states, mask):
        super().__init__(); self.skeleton=skeleton; self.mode=mode; self.mask=tuple(mask)
        self.adapters=nn.ModuleList()
        for i in range(3):
            self.adapters.append(NoOpAdapter() if not self.mask[i] else PhiAdapter(mode,base_states[i]))
        for p in self.skeleton.parameters(): p.requires_grad=False
    def forward(self,x):
        h=x; refs=[]
        for i,node in enumerate(self.skeleton.nodes):
            h=h+node(h); refs.append(h); ref=refs[-2] if i>0 else None; h=self.adapters[i](h,ref)
        return h
    def phi_parameters(self): return [p for p in self.parameters() if p.requires_grad]

class InterfaceWrappedRule(nn.Module):
    """Frozen primitive with tiny input/output task interfaces; primitive remains frozen."""
    def __init__(self,frozen_rule,input_mode='affine',output_mode='affine',use_input=True,use_output=True):
        super().__init__(); self.frozen_rule=frozen_rule; self.use_input=use_input; self.use_output=use_output
        self.input_iface=InterfaceAdapter(input_mode) if use_input else NoOpAdapter()
        self.output_iface=InterfaceAdapter(output_mode) if use_output else NoOpAdapter()
        for p in self.frozen_rule.parameters(): p.requires_grad=False
    def forward(self,x):
        h=self.input_iface(x) if self.use_input else x
        h=self.frozen_rule(h)
        return self.output_iface(h) if self.use_output else h
    def interface_parameters(self): return [p for p in self.parameters() if p.requires_grad]
...
def fit_interfaces(rule,teacher_bundle,args,device,steps):
    tea,tr,_=teacher_bundle; z,y=source_bundle(tea,tr,args,device); ps=rule.interface_parameters()
    if not ps: return rule
    opt=torch.optim.AdamW(ps,lr=args.interface_lr,weight_decay=args.interface_l2); z=z.to(device); y=y.to(device); rule.train()
    for _ in range(steps):
        ii=torch.randperm(len(z),device=device)[:min(args.fit_batch_samples,len(z))]
        pred=rule(z[ii]); yy=y[ii]
        loss=((pred-yy)**2).mean()+args.interface_l2*sum(p.square().mean() for p in ps)
        opt.zero_grad(set_to_none=True); loss.backward(); nn.utils.clip_grad_norm_(ps,0.5); opt.step()
    return rule

def interface_norm(rule):
    ps=rule.interface_parameters()
    return float(torch.sqrt(sum((p.detach()**2).sum() for p in ps)).item()) if ps else 0.0

def interface_param_count(rule): return sum(p.numel() for p in rule.interface_parameters())
...
def evaluate_frozen_transfer(teacher_bundle,best,base_states,args,device,control_primitive=None):
    ...
    frozen=make_frozen_rule(best['skeleton'],best['operator_mode'],base_states,best['placement'],device)
    configs=[
        ('dart_zero','identity','identity',False,False),
        ('dart_input',args.interface_mode,'identity',True,False),
        ('dart_output','identity',args.interface_mode,False,True),
        ('dart_both',args.interface_mode,args.interface_mode,True,True),
    ]
    acc={}; norms={}; counts={}
    for name,im,om,ui,uo in configs:
        r=InterfaceWrappedRule(copy.deepcopy(frozen),im,om,ui,uo).to(device)
        if ui or uo: r=fit_interfaces(r,teacher_bundle,args,device,args.target_interface_fit_steps)
        acc[name]=evaluate(RoutingCompiled(tea,r,args.trajectory_start,args.trajectory_end).to(device),test,device)
        norms[name]=interface_norm(r); counts[name]=interface_param_count(r)
    # Primitive permutation control.
    perm_base=[None,None,None]
    for s in range(3):
        if best['placement'][s]: perm_base[s]=best['rules'][0].adapters[s].state_dict()['raw'].detach().cpu()
    perm_f=make_frozen_rule(best['skeleton'],best['operator_mode'],perm_base,best['placement'],device)
    perm=InterfaceWrappedRule(perm_f,args.interface_mode,args.interface_mode,True,True).to(device)
    perm=fit_interfaces(perm,teacher_bundle,args,device,args.target_interface_fit_steps)
    perm_acc=evaluate(RoutingCompiled(tea,perm,args.trajectory_start,args.trajectory_end).to(device),test,device)
    # Random primitive control, retaining exactly the same interface budget.
    rs=SharedSkeleton(best['nodes'],best['motif'],args.d_model,args.rank).to(device)
    rand=InterfaceWrappedRule(make_frozen_rule(rs,best['operator_mode'],base_states,best['placement'],device),args.interface_mode,args.interface_mode,True,True).to(device)
    rand=fit_interfaces(rand,teacher_bundle,args,device,args.target_interface_fit_steps)
    rand_acc=evaluate(RoutingCompiled(tea,rand,args.trajectory_start,args.trajectory_end).to(device),test,device)
    teacher_acc=evaluate(tea,test,device)
    mlp=MLPControl(tea,args.trajectory_start,args.trajectory_end,args.mlp_width).to(device); train(mlp,tr,device,args.transfer_control_steps,args.lr); mlp_acc=evaluate(mlp,test,device)
    return {
        'teacher':teacher_acc,'dart_zero':acc['dart_zero'],'dart_input':acc['dart_input'],'dart_output':acc['dart_output'],'dart_both':acc['dart_both'],
        'primitive_permutation_control':perm_acc,'random_primitive_control':rand_acc,'mlp_control':mlp_acc,
        'gain_zero':(acc['dart_zero']-teacher_acc)*100,'gain_input':(acc['dart_input']-teacher_acc)*100,'gain_output':(acc['dart_output']-teacher_acc)*100,'gain_both':(acc['dart_both']-teacher_acc)*100,
        'vs_mlp_both':(acc['dart_both']-mlp_acc)*100,'interface_norm':norms['dart_both'],'interface_params':counts['dart_both'],
    }
...
def run_seed(args,seed,seed_idx=1,total_seeds=1):
    ...
    tracker.show(0.98,'frozen-transfer',f'target={target} contrast={contrast} iface={transfer["interface_norm"]:.3f}')
...
def main():
    p=argparse.ArgumentParser(description='DART-2.9 causal interface reparameterization around a frozen primitive + strict transfer')
    ...
    summary={'related_holdout':{args.holdout_tasks[0]:{k:avg('related_holdout',k) for k in ['teacher','dart_zero','dart_input','dart_output','dart_both','primitive_permutation_control','random_primitive_control','mlp_control','gain_zero','gain_input','gain_output','gain_both','vs_mlp_both','interface_norm','interface_params']}},
              'contrast_holdout':{args.contrast_tasks[0]:{k:avg('contrast_holdout',k) for k in ['teacher','dart_zero','dart_input','dart_output','dart_both','primitive_permutation_control','random_primitive_control','mlp_control','gain_zero','gain_input','gain_output','gain_both','vs_mlp_both','interface_norm','interface_params']}},
              'source':{
                ...
                'avg_tiny_interface_params':sum(r['related_holdout']['interface_params'] for r in rec)/len(rec),
                'avg_interface_norm':sum(r['related_holdout']['interface_norm'] for r in rec)/len(rec)}}
    payload={'config':vars(args),'records':rec,'summary':summary}
    Path(args.out).write_text(json.dumps(payload,indent=2)); print('DART-2.9: causal interface reparameterization around a frozen primitive + strict transfer'); print(json.dumps(summary,indent=2)); print('Saved:',Path(args.out).resolve())
